...
# 2. Define a dictionary containing the weights for each of the layers in charge of
# extracting style features:
style_weights = {'conv1_1': 1., 'conv2_1': 0.8, 'conv3_1': 0.6,
                 'conv4_1': 0.4, 'conv5_1': 0.2}
# Be sure to use the same names that you gave your layers in the previous
# exercise as keys.
...
# Define the number of iteration steps, as well as the optimization algorithm. We
# can also set the number of iterations if we want to see a plot of the image that
# has been created at that point:
print_statement = 200
optimizer = torch.optim.Adam([target_img], lr=0.001)
iterations = 2000

# The parameters to be updated by this optimization algorithm should be the
# parameters of the target image.

# Note
# Running 2,000 iterations, as in the example in this exercise, will take quite
# some time, depending on your resources. However, to reach an outstanding
# target image, even more iterations may be required.
# For you to appreciate the changes that occur to the target image in each
# iteration, a couple of iterations will suffice, but you are encouraged to try
# training for longer

# 5. Define the for loop where all three loss functions will be calculated and the
# optimization process will be performed:
for i in range(1, iterations + 1):
    ...
    for layer in style_weights:
        # Create gram matrix for the layer
...
# 6. Plot both the content and target images to compare the results. This can be
# achieved by using the tensor2image function, which we created in the
# previous exercises, in order to convert the tensors into PIL images that can be
# printed using matplotlib:
